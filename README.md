...
- **Wallpapers** — `~/Pictures/wallpapers` (lowercase, matches `CAELESTIA_WALLPAPERS_DIR`), `~/.face` avatar. Optional: `WALLPAPER_BANK=1 ./install.sh` downloads the JaKooLit Wallpaper-Bank image pack (~1 GB) into that folder via `fetch-wallpaper-bank.sh` — image files only, nothing JaKooLit-branded is installed or vendored in this repo.
- **Hyprland autostart** — comments out Waybar/AGS/`qs -c overview`/`hypridle`/`swaync`/`WallpaperDaemon`/`ApplyThemeMode` in `~/.config/hypr/configs/Startup_Apps.conf`, adds `exec-once = caelestia shell -d` to `~/.config/hypr/UserConfigs/Startup_Apps.conf`, injects startup env wrapper so the shell inherits `HYPRLAND_INSTANCE_SIGNATURE` (see Night Light). Disables/masks `waybar.service`/`ags.service`/`hypridle.service`/`swaync.service`/`hyprpaper.service`.
- **JaKooLit UI stack removal** — Caelestia owns lock (`hyprlock` configs + trigger removed), idle (`hypridle` removed), notifications (`swaync` → shell registers `org.freedesktop.Notifications`), wallpaper (`hyprpaper`/`awww`/`swww`/`wallust` removed; shell renders wallpaper), power menu (`wlogout` → session drawer), bars (`waybar`/`ags` configs removed), JaKooLit `quickshell` overview remnants and rofi menu stack. Deletes `~/.config/{swaync,waybar,waybar-weather,ags,wlogout,wallust,rofi}`, keeps only `caelestia.jsonc` in `fastfetch`, adopts official `btop.conf` (`config/btop/`), drops the JaKooLit one-shot `initial-boot.sh` hook.
...
- **Keybinds** — appends to `~/.config/hypr/UserConfigs/UserKeybinds.conf`: `Super+Space` launcher (`drawers toggle launcher`), `Super+N` nexus, `Super+Shift+E` session, `Super+L` lock, `Super+/` keybinds cheatsheet; `Super+W` random wallpaper, `Super+Shift+W` dark/light toggle (`~/.config/hypr/scripts/toggle-scheme.sh`). Unbinds JaKooLit rofi/wallust/waybar/swaync menu binds (app launcher `Super+D`, overview `Super+A`, web search `Super+S`, hints `Super+H`, emoji, music, theme/wallpaper pickers, waybar toggles, swaync panel, quick settings) and rebinds official caelestia keys: `Super+T` terminal, `Super+C` editor (nvim), `Ctrl+Alt+P` power menu → session drawer.
