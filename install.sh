...
# Reload Hyprland after applying colours so dynamic border/decor colours update live
hook = "$HOME/.config/caelestia/kitty-colors.py; hyprctl reload >/dev/null 2>&1"
...
# Decoration colours: point UserDecorations.conf at the caelestia-generated scheme palette
if [[ -f ~/.config/hypr/UserConfigs/UserDecorations.conf ]]; then
    sed -i 's|^source = $HOME/.config/hypr/wallust/wallust-hyprland.conf|# Sourcing colors generated by caelestia scheme (was: JaKooLit wallust palette)\nsource = $HOME/.config/hypr/scheme/current.conf|' ~/.config/hypr/UserConfigs/UserDecorations.conf 2>/dev/null || true
    sed -i 's|col.active_border = $color12|col.active_border = rgb($primary)|; s|col.inactive_border = $color10|col.inactive_border = rgb($outlineVariant)|' ~/.config/hypr/UserConfigs/UserDecorations.conf 2>/dev/null || true
    sed -i 's|color =  $color12|color =  rgb($shadow)|; s|color_inactive = $color10|color_inactive = rgb($scrim)|' ~/.config/hypr/UserConfigs/UserDecorations.conf 2>/dev/null || true
    sed -i 's|col.border_active = $color15|col.border_active = rgb($primary)|; s|col.active = $color0|col.active = rgb($primary)|' ~/.config/hypr/UserConfigs/UserDecorations.conf 2>/dev/null || true
    # Regenerate the scheme palette file so border/decor colours resolve at first Hyprland start
    if command -v caelestia &>/dev/null; then
        MODE="$(caelestia scheme get 2>/dev/null | awk '/Mode:/{print $2}')"
        [[ -n "$MODE" ]] && caelestia scheme set -m "$MODE" >/dev/null 2>&1 || true
    fi
    ok "UserDecorations now themed by the dynamic caelestia scheme"
fi
# Remaining dormant JaKooLit artefacts: wallust palette dir, unused Lua config layer (conf mode is active)
rm -rf "$HOME/.config/hypr/wallust"
rm -f "$HOME/.config/hypr/hl.meta.lua" "$HOME/.config/hypr/.luarc.json"
rm -rf "$HOME/.config/hypr/lua"
rm -f "$HOME/.config/hypr/configs"/*.lua "$HOME/.config/hypr/UserConfigs"/user_*.lua 2>/dev/null || true
if [[ -f ~/.config/hypr/configs/Startup_Apps.conf ]]; then
    sed -i 's|^exec-once = \$scriptsDir/LuaAutoReload.sh$|# Disabled by caelestia installer: Lua config layer removed (conf mode)|' ~/.config/hypr/configs/Startup_Apps.conf 2>/dev/null || true
fi
